...
private:
  ...
  volatile block_t
      *blocks; // the array of voxels by palette index (see block.h for more
...
public:
  world_page(int x0_, int y0_, int z0_, int dim_, world *wld_)
      ...
      : x0(x0_), y0(y0_), z0(z0_), blocks(nullptr), light(nullptr),
        zmap(nullptr), dirty_list(nullptr), invisible_blocks(nullptr),
        dim(dim_), nulled(false), loaded(false), wld(wld_) {
    assert(dim > 0);
  }

  world_page()
      : blocks(nullptr), light(nullptr), dirty_list(nullptr), zmap(nullptr),
        invisible_blocks(nullptr), dim(0), nulled(true), loaded(false),
        wld(nullptr) {}
  ...
  bool set(int x, int y, int z, block_t b, bool update_rle = true,
           bool update_neighborhood = true);
  bool set_unsafe(int x, int y, int z, block_t b, bool update_rle = true,
                  bool update_neighborhood = true);
  void reset_diff();
...
public:
  world *get_world();
  world_view(world *w, Vector3 center0, int rad, job_pool *workers_)
      : center{(int)(center0.x() / constants::CHUNK_WIDTH),
               (int)(center0.y() / constants::CHUNK_WIDTH), (int)center0.z()},
        center_old{(int)(center0.x() / constants::CHUNK_WIDTH),
                   (int)(center0.y() / constants::CHUNK_WIDTH),
                   (int)center0.z()},
        wld{w}, radius{rad}, mesh_update_head{0}, fast_update_head{0},
...
class world_light{
public:
    static void calculate_block_shadow(block_t *column, uint16_t
*skip_neighbors, uint16_t *z_neighbors, int ix, int iy, int x, int y, world_page
*page, world *wld) ;
};*/
