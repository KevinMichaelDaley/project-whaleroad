...
struct packet_block_update {
  uint8_t tick_index;
  uint16_t block_update_index; // if this increments by more than one, we know
                               // we lost something and need to re-request it.
...
struct packet_column_rle {
  uint8_t tick_index;
  int32_t x0, y0;
}; // N runs will be read sequentially from the next N*4 bytes using the
   // following struct:
...
struct packet_entity_state_update {
  ...
  uint16_t entity_id_lo; // lowest two bits reserved for sign of orientation
                         // w-component and look vector forward (y) component in
                         // local space
  uint8_t entity_id_hi;
  uint8_t anim_state; // for animation, etc.
  int x, y;
  uint16_t z; // block coordinates must be absolute in every update; if send
              // deltas and we lose a packet, everything gets messed up.
  uint8_t xsub, ysub, zsub; // 0-1 normalized 8-bit value for offset from lower
                            // left bottom corner of block.
...
enum class message_server : char { MSG_BLK_UPD, MSG_RLE, MSG_REQ, MSG_ENT_UPD };
enum class message_client : char { MSG_BLK_UPD, MSG_CHKSUM, MSG_REQ, MSG_ACT };
...
class udp_transmitter {
  ...
  udp_transmitter(udp_connection *conn_, int max_dgram_size)
      : conn(conn_), max_datagram_size(max_dgram_size), num_bytes_filled(0),
        current_tick(0), time_last_flush(0) {
...
class client : public udp_transmitter {
  ...
  client(udp_connection *srv_, int udp_mtu)
      : udp_transmitter(srv_, udp_mtu - constants::UDP_HEADER_SIZE -
  ...
  void unpack_update(char *dgram, int &offset) {
    if (dgram[0] & 3 == message_server::MSG_BLK_UPD) {
      ...
      packet_physics_state_update x =
          *(packet_physics_state_update *)&(dgram[offset]);
...
class server {
  ...
  static packet_entity_state_update pack_entity_state(const character *ent) {
    ...
    id += (q.scalar() > 0) << 1;
    Vector3 look =
        q.invertedNormalized().transformVectorNormalized(Vector3{rx, ry, rz});
    float look_x = (look.x() / look.length());
    float look_z = (look.z() / look.length());
    id += (look.y() > 0);
    Vector3 qv = q.vector();
    int bx = std::floor(pos.x()), by = std::floor(pos.y()),
        bz = std::floor(pos.z());
    packet_physics_state_update upd{
        uint8_t(current_tick % 64u) << uint8_t(2u) + uint8_t(3u),
        Float16Compressor::compress(
            std::fmod(timer::now(), constants::NETWORK_TIMER_RESET_INTERVAL) /
            constants::NETWORK_TIMER_RESET_INTERVAL),
        id & 65535,
        id >> 16,
        ent->get_anim_state(),
        bx,
  ...
  virtual void unpack_update(char *dgram, int &offset) {
    if (dgram[0] & 3 == message_server::MSG_BLK_UPD) {
      ...
      packet_player_input_event x =
          *(packet_player_input_event *)&(dgram[offset]);
