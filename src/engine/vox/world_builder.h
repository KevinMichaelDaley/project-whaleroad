...
public:
  enum map_mode { MAP_READ, MAP_WRITE };
...
public:
  ...
  ~world_builder() { unmap_world_file(); }
};
...
public:
    ...
    for (int r = 29; r > 0; r -= 1) {
      ...
      for (int dx = 0; dx <= 1; dx += 1) {
        for (int dy = 0; dy <= 1; dy += 1) {
          ...
          int y2 = yd + dy;
          int bit = (x2 & 1) * 2 + (y2 & 1);
          // seed with position hash (to preserve spatial coherence) and do lcg
          // once
          pattern[dx][dy] =
              !!((((x2 / 2) * 65539 + (y2 / 2)) * 48271) & (1 << bit));
        }
      }
      // printf("%f %f\n",s,t);
      /*float off_interp0=(1-s)*pattern[0][0]+s*pattern[1][0];
      float off_interp2=(1-s)*pattern[0][1]+s*pattern[1][1];
      float off_interp = ((1-t)*(off_interp0)+(t)*(off_interp2));*/
      height += pattern[0][0] * tanh(r * r / (1024.0));
  ...
  float gen_height(int x, int y) { return gen_pattern(x + x0, y + y0); }
  ...
  void get_generated_run(int x, int y, int z, block_t *b, int *n) {
    ...
    if (method.compare("default") == 0) {
      ...
      int h = std::max(
          0, std::min(constants::WORLD_HEIGHT - 64,
                      (int)(gen_height(x, y) * WORLD_SCALE + WORLD_OFFSET)));
      ...
      if (z < (int)(h)) {
        ...
        for (int i = -1; i <= 1; ++i) {
          for (int j = -1; j <= 1; ++j) {
            ...
            int h2 = std::max(
                0, std::min(constants::WORLD_HEIGHT - 64,
                            (int)(gen_height(x + i, y + j) * WORLD_SCALE +
                                  WORLD_OFFSET)));
  ...
  worldgen_stream(std::string m, stream_mode md, int ox, int oy, int oz) {
    ...
    if (method.compare("default")) {
      so = dlopen(
          ("../../src/engine/proc/terrain/gen_" + method + ".so").c_str(),
          RTLD_NOW);
  ...
  virtual size_t read(char *output, size_t size) {
    ...
    for (size_t off = 0; off < size / sizeof(block_t); off += 2u) {
      ...
      int xz = 0;
      get_generated_run(ij / constants::PAGE_DIM, ij % constants::PAGE_DIM, k,
                        &b, &n);
