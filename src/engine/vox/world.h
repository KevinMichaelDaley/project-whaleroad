...
private:
  ...
  unsigned long long dim; // this is the size of the page in both x and y
...
public:
  world_page(int x0_, int y0_, int z0_, int dim_)

      : x0(x0_), y0(y0_), z0(z0_), blocks(nullptr), light(nullptr),
        dirty_list(nullptr), dim(dim_), nulled(false) {
    assert(dim > 0);
  }

  world_page() { nulled = true; }
...
public:
  world *get_world() ;
  world_view(world* w, Vector3 center0, int rad): center{center0.x(), center0.y(), center0.z()},
                                                    center_old{center0.x(), center0.y(), center0.z()}, 
                                                    wld{w},
                                                    radius{rad},
                                                    mesh_update_head{0},
                                                    first_frame{true}
