...
public:
  block_default_forward_pass(GL::Texture2D &atlas)
      : GBuffer(GL::defaultFramebuffer.viewport()), deferred{"deferred"},
        passthrough{"passthrough"}, gbuffer{"gbuffer"}, sky{"sky"} {
    frame = 0;
    ...
    const Vertex quadData[]{lb, rt, lt, rt, lb, rb};

    _quadBuffer.setData(quadData, GL::BufferUsage::StaticDraw);

    fsquad.setPrimitive(GL::MeshPrimitive::Triangles)
        .setCount(6)
        .addVertexBuffer(_quadBuffer, 0, pos{}, uv{});
    ...
    fullchunk.setPrimitive(GL::MeshPrimitive::Triangles)
        .setCount(12)
        .addVertexBuffer(_cubeBuffer, 0, cpos{});
    ...
    .setMagnificationFilter(GL::SamplerFilter::Linear);
            (shadowFramebuffer[l]=new GL::Framebuffer{Range2Di{{},
{SMAP_RES(l),SMAP_RES(l)}}})->attachTexture(GL::Framebuffer::ColorAttachment(0),
shadowMap[l], 0)

        .attachRenderbuffer(
GL::Framebuffer::BufferAttachment::DepthStencil, depthStencil[l])
        .mapForDraw({{ 0, {GL::Framebuffer::ColorAttachment( 0 )} }});
         auto status =  shadowFramebuffer[l]->checkStatus(
Magnum::GL::FramebufferTarget::Draw ); if( status !=
Magnum::GL::Framebuffer::Status::Complete )
    ...
    */
#ifndef __ANDROID__
    depthStencil1.setStorage(GL::RenderbufferFormat::DepthStencil,
                             {GL::defaultFramebuffer.viewport().sizeX(),
                              GL::defaultFramebuffer.viewport().sizeY()});
#else
    depthStencil1.setStorage(GL::RenderbufferFormat::Depth24Stencil8,
                             {GL::defaultFramebuffer.viewport().sizeX(),
                              GL::defaultFramebuffer.viewport().sizeY()});
#endif
    (gbuffertex = GL::Texture2D{})
        .setStorage(1, GL::TextureFormat::RGBA16F,
                    {GL::defaultFramebuffer.viewport().sizeX(),
                     GL::defaultFramebuffer.viewport().sizeY()})
        .setMinificationFilter(GL::SamplerFilter::Linear)
        .setMagnificationFilter(GL::SamplerFilter::Linear);
    GBuffer.attachTexture(GL::Framebuffer::ColorAttachment(0), gbuffertex, 0)
        .attachRenderbuffer(GL::Framebuffer::BufferAttachment::DepthStencil,
                            depthStencil1)
        .mapForDraw({{0, {GL::Framebuffer::ColorAttachment(0)}}});
  ...
  float get_day_length(float year_offset) { return 0.8; }
  float get_sky_angle(Vector3 &sky_color, Vector3 &sun_color, bool &is_day) {
    ...
    float SECONDS_IN_DAY =
        get_day_length(day_index / (float)DAYS_IN_YEAR) * SECONDS_IN_DAY_NIGHT;
    ...
    if (is_day) {
      ...
      float theta =
          std::fmod(time_since_sunrise, SECONDS_IN_DAY) / SECONDS_IN_NIGHT;
  ...
  camera get_sun_cam(int i, int j) {
    ...
    cam2.set_ortho({SHADOW_DIST, SHADOW_DIST},
                   0.1 + (SHADOW_DIST * 2.0) * i / double(SHADOW_CASCADES),
                   0.1 + (SHADOW_DIST * 2.0) * (j) / double(SHADOW_CASCADES));
    Vector3 tmp1, tmp2;
    bool tmp3;
    float t = get_sky_angle(tmp1, tmp2, tmp3);
    cam2.look_at({100 * (x / 100) + SHADOW_DIST * sin(t), 100 * (y / 100),
                  SHADOW_DIST * cos(t)},
                 {-sin(t), 0.0, -cos(t)}, {cos(t), 0.0, sin(t)});
    return cam2;
  }
  float cross(Vector2 x, Vector2 y) { return x.x() * y.y() - x.y() * y.x(); }
  void drawTri(uint16_t *cull_array, int res, Vector3 vt1, Vector3 vt2,
               Vector3 vt3, int i) {
    ...
    for (int x = minX; x <= maxX; x += 1) {
      for (int y = minY; y <= maxY; y++) {
        if (x < -res / 2 || x >= res / 2 || y < -res / 2 || y >= res / 2)
          continue;
        ...
        if ((s >= 0) && (t >= 0) && (s + t <= 1)) {
          ...
          uint16_t z1 = std::min(uint16_t(254u),
                                 (uint16_t)(255.0f * (0.5f / invz + 0.5f)));
  ...
  void rasterize(chunk_mesh *chunk, int z0, camera cam, uint16_t *cull_array,
                 int res, uint8_t index, bool whole_box_only = true) {
    Matrix4 viewproj = cam.projection * cam.view;
    int x0 = chunk->x0;
    int y0 = chunk->y0;
    int N = constants::CHUNK_HEIGHT * constants::CHUNK_WIDTH *
            constants::CHUNK_WIDTH;
    bool full =
        (chunk->volume == constants::CHUNK_WIDTH * constants::CHUNK_HEIGHT *
                              constants::CHUNK_WIDTH);
    ...
    if (full) {
      for (int face = 0; face < 6; ++face) {
        ...
        for (int vert = 0; vert < 4; ++vert) {
          Vector4 vi =
              viewproj *
              (Vector4{x0 + FacesOffset[face][vert][0] * constants::CHUNK_WIDTH,
                       y0 + FacesOffset[face][vert][1] * constants::CHUNK_WIDTH,
                       z0 +
                           FacesOffset[face][vert][2] * constants::CHUNK_HEIGHT,
                       1.0});
    ...
    for (int i = 0, e = chunk->Nverts[z0]; i < e; ++i) {
      ...
      for (int face = 0; face < 6; ++face) {
        ...
        for (int vert = 0; vert < 4; ++vert) {
          Vector4 vi =
              viewproj * (Vector4{x + FacesOffset[face][vert][0],
                                  y + FacesOffset[face][vert][1],
                                  z + FacesOffset[face][vert][2], 1.0});
          v[vert] = Vector3{vi.x(), vi.y(), vi.z()} / vi.w();
        }
        drawTri(cull_array, res, v[1], v[3], v[0], index);
        drawTri(cull_array, res, v[2], v[0], v[1], index);
  ...
  block_default_forward_pass &draw_world_view(world_view *wv) {
    ...
    while (q.size() < all_chunks.size() *
                          (constants::WORLD_HEIGHT / constants::CHUNK_HEIGHT)) {
      q.push_back(
          new GL::SampleQuery{GL::SampleQuery::Target::AnySamplesPassed});
    }

    passthrough.uniform("transform", viewproj);
    std::sort(indices.begin(), indices.end(), [this](int i, int j) {
      return this->min_depth[i] < this->min_depth[j];
    });
   ...
   for(int l=0; l<SHADOW_CASCADES; ++l){
      ...
      // GL::Renderer::enable(GL::Renderer::Feature::FaceCulling);
       ...
       for(int i=0,e=all_chunks.size(); i<e; ++i){
               ...
               for(int z0=0; z0<constants::WORLD_HEIGHT/constants::CHUNK_HEIGHT;
   ++z0){ if(!chunk->is_visible(slice_cam,z0)) continue; int
   ixc=i*constants::WORLD_HEIGHT/constants::CHUNK_HEIGHT+z0;
   ...
   */
    ...
    for(int i=0,e=all_chunks.size(); i<e; ++i){
        ...
        for(int z0=0; z0<constants::WORLD_HEIGHT/constants::CHUNK_HEIGHT; ++z0){
            ...
            int ixc=i*constants::WORLD_HEIGHT/constants::CHUNK_HEIGHT+z0;
            std::swap(q[ixc],
    q[ixc+constants::WORLD_HEIGHT*all_chunks.size()/constants::CHUNK_HEIGHT]);
        }
    }*/
    return *this;
  }
